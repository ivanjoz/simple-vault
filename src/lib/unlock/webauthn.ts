...
import { importAesKey, randomBytes } from '$lib/crypto';
...
export async function enrollWebAuthn(): Promise<WebAuthnEnrollment | null> {
	...
	const credentialId = new Uint8Array(cred.rawId) as Bytes;
...
export async function unlockWebAuthn(credentialId: Bytes, prfSalt: Bytes): Promise<CryptoKey> {
	const output = await evaluatePrf(credentialId, prfSalt);
	return importAesKey(new Uint8Array(output));
}

async function evaluatePrf(credentialId: Bytes, prfSalt: Uint8Array): Promise<ArrayBuffer> {
